...
**Date:** 2026-07-21
**Status:** `0.37.0` is released with the RFC-071 fixed-demo private JSON report slice. RFC-070 is closed without a public report/viz crate or API. RFC-072 design is accepted as the authority for behavior-preserving `matten-report` modularization. The next action is a detailed implementation handoff defining the distinct pre-movement process baseline and the family/format-sized movement units. No harness or Rust source movement is authorized before that handoff is reviewed. Input-mode JSON, broader mathematics, ecosystem bridges, public schemas, public renderer APIs, and public report/viz crates remain unauthorized.
