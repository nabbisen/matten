...
| **Post-v0.38.0** | v1.0 release decision | RFC-075 proposed: resolves MD-2 (keeps RFC-030 lock-step versioning, adds a CHANGELOG justification requirement for local-tool-only releases), declares the JSON canonical serde format stable, records the RFC-067 family maturity table (matten-mlprep/matten-data included at candidate label, not promoted) | Decision-only; a separate future v1.0 release-prep RFC required before any release action |
