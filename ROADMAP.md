...
**Date:** 2026-07-22
**Status:** `0.37.0` is released with the RFC-071 fixed-demo private JSON report slice. RFC-070 is closed without a public report/viz crate or API. RFC-072 Slice 0, Slice 1, and the Phase 2 shape-flow, dynamic-readiness, and MLPrep-standardization family extractions are reviewed and committed. The educational-path family unit is implemented and prepared for review: `report::educational_path` owns its composite Tensor, dynamic-readiness, and standardization computation; app constructs one value before format dispatch; and Markdown/HTML/JSON renderers borrow it. Educational renderer tests are separated and both transitional renderer files shrink again. Data-readiness and later format units remain unauthorized until this checkpoint is reviewed and committed. Input-mode JSON, broader mathematics, ecosystem bridges, public schemas, public renderer APIs, and public report/viz crates remain unauthorized.
...
| **Post-v0.37.0** | RFC-070 closure and report-tool maintainability | RFC-070 closed without a public report/viz surface; RFC-072 Slice 0/1 plus shape-flow, dynamic-readiness, and MLPrep-standardization extractions committed; educational-path family extraction prepared for review | Implementation checkpoint |
...
and installs the durable module-boundary gate. The first three Phase 2 family
units, shape-flow, dynamic-readiness, and MLPrep-standardization, are reviewed
and committed; educational-path is implemented and prepared for review before
the final data-readiness family extraction.
