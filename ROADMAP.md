...
| **Post-v0.38.0** | Pre-v1 feature work | RFC-077 (`train_test_split_seeded` for `matten-mlprep`, closing the RFC-076 §5 maturity caveat without promoting the crate) implemented, reviewed (GO, no conditions), and closed; RFC-078 (`matten-stats` companion proposal) proposed on the `0.38.x` line while RFC-076 stays accepted-but-unexecuted | RFC-077 closed; RFC-078 not yet reviewed; no version bump for either theme |
