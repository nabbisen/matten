...
**Date:** 2026-07-22
**Status:** `0.37.0` is released with the RFC-071 fixed-demo private JSON report slice. RFC-070 is closed without a public report/viz crate or API. RFC-072 Slice 0, Slice 1, and the Phase 2 shape-flow family extraction are reviewed and committed. The dynamic-readiness family unit is implemented and prepared for review: `report::dynamic_readiness` owns dynamic tensor computation, schema data, and element normalization; app constructs one value before format dispatch; and Markdown/HTML/JSON renderers borrow it. Dynamic-readiness renderer tests are separated and both transitional renderer files shrink again. MLPrep standardization and all later family/format units remain unauthorized until this checkpoint is reviewed and committed. Input-mode JSON, broader mathematics, ecosystem bridges, public schemas, public renderer APIs, and public report/viz crates remain unauthorized.
...
| **Post-v0.37.0** | RFC-070 closure and report-tool maintainability | RFC-070 closed without a public report/viz surface; RFC-072 Slice 0/1 and shape-flow extraction committed; dynamic-readiness family extraction prepared for review | Implementation checkpoint |
...
and installs the durable module-boundary gate. The first Phase 2 family unit,
shape-flow, is reviewed and committed; dynamic-readiness is implemented and
prepared for review before any later family extraction.
