...
**Document Version:** `1.67.0`
**Date:** 2026-07-15
**Status:** Canonical roadmap updated for the 0.32.0 RFC-068 release-prep slice. The release scope is local static HTML artifacts for `tools/matten-report --demo educational-path` and `tools/matten-report --demo shape-flow`, with Markdown/plain text still default and all other report families/input mode out of HTML scope. This does not authorize public report/viz crates, core visualization APIs, expression tracing, autograd, dependency changes in published crates, SVG/Vega-Lite/JSON/notebook/browser scope, tag, publish, or companion maturity changes.
