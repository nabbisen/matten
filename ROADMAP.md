...
| **v0.39.0** | Pre-v1 feature release (release-prep only) | RFC-079 reviewed and accepted (GO, no conditions): bumps the lock-step family version `0.38.0` -> `0.39.0`, retargets current-family version strings, and adds a CHANGELOG entry releasing `train_test_split_seeded` (RFC-077). `matten-stats` (RFC-078) is version-bumped in lock-step but its first publication is deferred pending an external read of RFC-078 §4.1's `ddof = 1` policy, obtained outside this project's assistant session — an owner decision, not a review finding | Release-prep reviewed; **publish and tag remain a separate owner-authorized step**; `matten-stats` publish excluded from that step |
