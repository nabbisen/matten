...
**Document Version:** `1.66.0`
**Date:** 2026-07-15
**Status:** Canonical roadmap updated for the RFC-068 shape-flow HTML implementation-review slice. The implementation extends local static HTML output to exactly one additional fixed report family, `tools/matten-report --demo shape-flow`, while preserving Markdown as default and keeping all other report families/input mode out of HTML scope. This does not authorize public report/viz crates, core visualization APIs, expression tracing, autograd, dependency changes in published crates, SVG/Vega-Lite/JSON/notebook/browser scope, version bump, tag, publish, or companion maturity changes.
