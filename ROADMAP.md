...
**Document Version:** `1.72.0`
**Date:** 2026-07-15
**Status:** Canonical roadmap updated for post-0.33.0 RFC-068 visualization-continuation planning. The next recommended review slice is one local static HTML artifact for `tools/matten-report --demo mlprep-standardization --format html --output <path>`, with Markdown/plain text still default and `data-readiness` plus input-mode HTML still out of scope. This planning work does not authorize implementation until review acceptance, and does not authorize public report/viz crates, core visualization APIs, expression tracing, autograd, dependency changes in published crates, SVG/Vega-Lite/JSON/notebook/browser scope, version bump, tag, publish, or companion maturity changes.
