...
| **Post-v0.37.0** | Report-tool policy and maintainability | RFC-070 public-readiness audit and RFC-072 modularization closed; RFC-073 policy/handoff accepted and one private input-mode JSON implementation checkpoint authorized; release undecided | Implementation checkpoint |
...
RFC-073 policy and detailed handoff are accepted for bounded private
data-readiness input-mode JSON. The handoff defines exact schema-v0 representation types, structured
user-data bounds, success/error report outcomes, pre-write destination
preservation, unchanged non-atomic write-time semantics, fixed-demo byte
preservation, a single `render::common` display-limit owner, and one authorized
implementation review checkpoint. Release work remains unauthorized. Public
JSON schemas, SVG, and Vega-Lite remain deferred.
