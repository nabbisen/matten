...
| **v0.40.0** | Feature and maturity release | RFC-086 reviewed and accepted (GO): bumps the lock-step family version `0.39.0` -> `0.40.0`, retargets 36 current-family version strings across 16 files, and publishes the accumulated content of RFC-082 through RFC-085 — `CsvBatchReader` and three new `matten-stats` functions, plus the `matten-mlprep`/`matten-stats`/`matten-data` maturity promotions, none of which had been user-visible before this release. Motivated by finding that no step in the RFC process ever asks whether an accumulation of individually-no-release slices has itself become release-worthy (RFC-086 §2); RFC-086 §10 proposes closing that gap as a future amendment, not adopted by this release | **Prepared, not tagged or published.** Tagging is blocked (not merely deferred): the `filter-branch` history rewrite left the `0.38.0` and `0.39.0` git tags orphaned, pointing at commits absent from `main` on both the local repo and `origin`. Re-tagging both correctly is the owner's action and a precondition to tagging `0.40.0` |
...
| Theme | Authority | State |
|---|---|---|
| `repeat` / `tile` / `meshgrid` | RFC-039 §8 | Deferred with the design work already done — the cheapest core function theme |
| `is_empty()` | compatibility.md | Deferred; zero-sized dims are rejected by the shape model, so it would always be false |
| Negative slice indices | RFC-008, compatibility.md | Deferred; not in the `0.1.0` slice grammar |
...
| Theme | Authority | State |
|---|---|---|
| Histogram | RFC-040 §8 | Blocked on an unresolved bin-selection policy. The unblocking RFC is a **decision**, not an implementation — cheap, and it converts a blocked item into an available one |
| Matrix-wide / axis-wise covariance and correlation | RFC-083 §6 | Would return a `Tensor`, changing the crate's stated `Tensor -> f64` shape; needs its own boundary RFC |
