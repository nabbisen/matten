...
**Date:** 2026-07-22
**Status:** `0.37.0` is released with the RFC-071 fixed-demo private JSON report slice. RFC-070 is closed without a public report/viz crate or API. RFC-072 Slice 0, Slice 1, and the Phase 2 shape-flow and dynamic-readiness family extractions are reviewed and committed. The MLPrep-standardization family unit is implemented and prepared for review: `report::mlprep_standardization` owns its Tensor and standardization computation; app constructs one value before format dispatch; and Markdown/HTML/JSON renderers borrow it. MLPrep renderer tests are separated and both transitional renderer files shrink again. Educational-path and all later family/format units remain unauthorized until this checkpoint is reviewed and committed. Input-mode JSON, broader mathematics, ecosystem bridges, public schemas, public renderer APIs, and public report/viz crates remain unauthorized.
...
| **Post-v0.37.0** | RFC-070 closure and report-tool maintainability | RFC-070 closed without a public report/viz surface; RFC-072 Slice 0/1 plus shape-flow and dynamic-readiness extractions committed; MLPrep-standardization family extraction prepared for review | Implementation checkpoint |
...
and installs the durable module-boundary gate. The first two Phase 2 family
units, shape-flow and dynamic-readiness, are reviewed and committed;
MLPrep-standardization is implemented and prepared for review before any later
family extraction.
