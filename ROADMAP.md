...
**Document Version:** `1.76.0`
**Date:** 2026-07-16
**Status:** Canonical roadmap updated for the post-0.34 RFC-068 data-readiness demo-only HTML handoff. The reviewed gap audit chose the conservative continuation path: draft a dedicated handoff for `tools/matten-report --demo data-readiness --format html --output <path>`. This authorizes no implementation until handoff review. Input-mode HTML, public report/viz crates, core visualization APIs, expression tracing, autograd, dependency changes in published crates, SVG/Vega-Lite/JSON/notebook/browser scope, version bump, tag, publish, and companion maturity changes remain deferred.
