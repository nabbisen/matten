...
**Document Version:** `1.80.0`
**Date:** 2026-07-16
**Status:** Canonical roadmap updated for proposed RFC-069 input-mode HTML report policy. RFC-068 fixed-demo HTML remains closed after 0.35.0; RFC-069 opens a separate review path for whether `tools/matten-report --input <csv> --kind data-readiness --select <cols> --format html --output <path>` may become a narrow, summary-only, bounded, escaped local artifact. No implementation, public report/viz crate, core visualization API, expression tracing, autograd, dependency change in published crates, SVG/Vega-Lite/JSON/notebook/browser scope, version bump, tag, publish, or companion maturity change is authorized.
...
| **Post-v0.35.0** | Input-mode HTML policy | RFC-069 proposed: decide whether local static HTML may be opened for `tools/matten-report --input ... --kind data-readiness` under summary-only, bounded, escaped, explicit-output rules | Policy audit only |
