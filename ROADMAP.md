...
**Date:** 2026-07-21
**Status:** `0.37.0` is released with the RFC-071 fixed-demo private JSON report slice. RFC-070 is closed without a public report/viz crate or API. RFC-072 Slice 0 is reviewed and committed. Slice 1 second remediation is prepared for rereview: grouped-import inspection now collects complete use statements, tracks nested brace depth, and rejects forbidden outer roots; visibility inspection rejects every `pub` form except `pub(crate)`. The full format block, 59 tests, normal/self-test guards, and unchanged process baseline pass. Report-family extraction and all later slices remain unauthorized until Slice 1 rereview and commit. Input-mode JSON, broader mathematics, ecosystem bridges, public schemas, public renderer APIs, and public report/viz crates remain unauthorized.
...
| **Post-v0.37.0** | RFC-070 closure and report-tool maintainability | RFC-070 closed without a public report/viz surface; RFC-072 Slice 0 committed and Slice 1 second remediation prepared for rereview | Implementation checkpoint |
...
and installs the durable module-boundary gate; it is prepared for review before
any report-family extraction.
