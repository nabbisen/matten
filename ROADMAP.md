...
**Date:** 2026-07-22
**Status:** `0.37.0` is released with the RFC-071 fixed-demo private JSON report slice. RFC-070 is closed without a public report/viz crate or API. RFC-072 implementation through Phase 3C HTML extraction is reviewed and committed. Phase 3D private JSON extraction is implemented and prepared for review: the private envelope/model, finite-value policy, and five family mappings/tests now live under `render::json`; a focused NaN/Infinity rejection test raises the total from 59 original tests to 60. All established report bytes and the 404-byte process fingerprint remain unchanged. Structural closure remains unauthorized until this JSON checkpoint is reviewed and committed. Input-mode JSON, broader mathematics, ecosystem bridges, public schemas, public renderer APIs, and public report/viz crates remain unauthorized.
...
| **Post-v0.37.0** | RFC-070 closure and report-tool maintainability | RFC-070 closed without a public report/viz surface; RFC-072 through Phase 3C committed; Phase 3D private JSON extraction prepared for review | Implementation checkpoint |
...
family/test owner are also reviewed and committed. Phase 3D moves the private
JSON model, finite-value policy, family mappings, and exact tests; it is
implemented and prepared for review before structural closure.
