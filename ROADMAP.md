...
**Document Version:** `1.77.0`
**Date:** 2026-07-16
**Status:** Canonical roadmap updated for the RFC-068 data-readiness demo-only HTML implementation-review slice. Local static HTML now covers all fixed `tools/matten-report --demo ...` reports for review, including `data-readiness`. Input-mode HTML remains rejected and requires a separate policy review. This slice does not authorize public report/viz crates, core visualization APIs, expression tracing, autograd, dependency changes in published crates, SVG/Vega-Lite/JSON/notebook/browser scope, version bump, tag, publish, or companion maturity changes.
