...
| **Post-v0.36.0** | Public visualization/report readiness | RFC-070 readiness audit drafted: public `matten-report` / `matten-viz` not ready; private report-model extraction implemented/reviewed/committed; JSON policy audit drafted, recommending private fixed-demo JSON handoff only if accepted | Audit / planning |
