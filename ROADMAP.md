...
**Document Version:** `1.65.0`
**Date:** 2026-07-15
**Status:** Canonical roadmap updated for the RFC-068 shape-flow HTML handoff after the shared educational report data refactor was committed. The next proposed slice extends the local static HTML artifact pattern to exactly one additional fixed report family, `tools/matten-report --demo shape-flow`, while preserving Markdown as default and keeping all other report families/input mode out of HTML scope. This does not authorize public report/viz crates, core visualization APIs, expression tracing, autograd, dependency changes in published crates, SVG/Vega-Lite/JSON/notebook/browser scope, version bump, tag, publish, or companion maturity changes.
...
| **v0.32.0 candidate** | Rich local visualization artifacts | RFC-068 proposed: static local HTML artifact for `tools/matten-report --demo educational-path`, preserving Markdown default and deferring public crates/SVG/Vega-Lite/expression tracing | Local-tool visualization artifact |
