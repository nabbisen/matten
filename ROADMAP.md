...
| **Post-v0.38.0** | v1.0 readiness re-audit | RFC-074 re-audit of RFC-066 complete (`docs/design/v1-readiness-audit.md`): public API/error/boundary surface conditionally ready (zero churn across eight releases); deferred stats/linalg found to be settled rejected-scope, not blockers; streaming remains additive, not a blocker; companion maturity promotion not evidenced; new maintainer-decision finding MD-2 on RFC-030 lock-step versioning's unfired RFC-071 §6 reconsideration trigger | Audit report prepared for review; no implementation, version bump, or release authorized |
