...
**Date:** 2026-07-16
**Status:** Canonical roadmap updated for the post-0.34 RFC-068 visualization gap audit. The 0.34.0 release remains the released state for the mlprep-standardization local HTML artifact. This audit authorizes no implementation; it asks review to choose whether to close the fixed-demo local HTML line or draft a dedicated demo-only `data-readiness` HTML handoff. Input-mode HTML, public report/viz crates, core visualization APIs, expression tracing, autograd, dependency changes in published crates, SVG/Vega-Lite/JSON/notebook/browser scope, and companion maturity changes remain deferred.
