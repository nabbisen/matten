...
**Date:** 2026-07-21
**Status:** `0.37.0` is released with the RFC-071 fixed-demo private JSON report slice. RFC-070 is closed without a public report/viz crate or API. RFC-072 Slice 0 and Slice 1 are reviewed and committed. The first Phase 2 family unit is implemented and prepared for review: `report::shape_flow` owns shape-flow computation/data, app constructs one value before format dispatch, and Markdown/HTML/JSON renderers borrow it. Shape-flow renderer tests are separated and both transitional renderer files shrink. Dynamic-readiness and all later family/format units remain unauthorized until this checkpoint is reviewed and committed. Input-mode JSON, broader mathematics, ecosystem bridges, public schemas, public renderer APIs, and public report/viz crates remain unauthorized.
...
| **Post-v0.37.0** | RFC-070 closure and report-tool maintainability | RFC-070 closed without a public report/viz surface; RFC-072 Slice 0/1 committed and shape-flow report-family extraction prepared for review | Implementation checkpoint |
