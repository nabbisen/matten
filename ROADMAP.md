...
**Date:** 2026-07-22
**Status:** `0.37.0` is released with the RFC-071 fixed-demo private JSON report slice. RFC-070 is closed without a public report/viz crate or API. RFC-072 Slice 0, Slice 1, and Phase 2 are reviewed and committed. The first Phase 3 checkpoint is implemented and prepared for review: format-neutral fixed-number formatting lives in `render::common`; data-readiness Markdown and list emission live in `render::markdown::data_readiness`; exact Markdown tests moved with that renderer; and selection-error tests moved to the report owner. All 59 tests and the 404-byte process fingerprint remain unchanged. Remaining Markdown families, HTML, JSON, and later structural work remain unauthorized until this checkpoint is reviewed and committed. Input-mode JSON, broader mathematics, ecosystem bridges, public schemas, public renderer APIs, and public report/viz crates remain unauthorized.
...
| **Post-v0.37.0** | RFC-070 closure and report-tool maintainability | RFC-070 closed without a public report/viz surface; RFC-072 Slice 0/1 and Phase 2 committed; shared formatting plus data-readiness Markdown extraction prepared for review | Implementation checkpoint |
...
is reviewed and committed. The first Phase 3 checkpoint moves shared numeric
formatting and data-readiness Markdown/test ownership; it is implemented and
prepared for review before the remaining Markdown families.
