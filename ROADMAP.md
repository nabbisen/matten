...
**Date:** 2026-07-22
**Status:** `0.37.0` is released with the RFC-071 fixed-demo private JSON report slice. RFC-070 is closed without a public report/viz crate or API. RFC-072 Slice 0, Slice 1, and the first four Phase 2 family extractions are reviewed and committed. The final Phase 2 data-readiness family unit is implemented and prepared for review: one report-owned model covers fixed and input data, app performs total non-panicking dispatch, and every renderer borrows report data. All transitional `unreachable!()` guards are removed, `render/tests.rs` is below the hard threshold, and all 59 tests remain. Phase 3 renderer extraction and later units remain unauthorized until this checkpoint is reviewed and committed. Input-mode JSON, broader mathematics, ecosystem bridges, public schemas, public renderer APIs, and public report/viz crates remain unauthorized.
...
| **Post-v0.37.0** | RFC-070 closure and report-tool maintainability | RFC-070 closed without a public report/viz surface; RFC-072 Slice 0/1 plus the first four Phase 2 families committed; final data-readiness extraction and total dispatch prepared for review | Implementation checkpoint |
...
and installs the durable module-boundary gate. The first four Phase 2 family
units are reviewed and committed. Final data-readiness extraction, unified
fixed/input report ownership, total app dispatch, and remaining test movement
are implemented and prepared for review before Phase 3 renderer extraction.
