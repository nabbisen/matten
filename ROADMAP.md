...
|---|---|---|
| `matten-data` → full production-ready | RFC-059 §6 | **Taken up as RFC-085.** RFC-059 deferred it pending "a separate future review after at least one candidate cycle", naming three concerns: newest companion, wide CSV edge-case surface, and streaming explicitly deferred. Streaming was discharged by RFC-082; the edge-case surface now has evidence (RFC-082's 4,000-case randomized differential run, zero mismatches); the candidate cycle has run since 2026-06-27 |
...
| `check-release-docs.sh:89` blanket `experimental` grep for `matten-data` | RFC-084 review C1 | Same over-broad shape C1 removed for `matten-stats`; harmless today only because `matten-data` was never at that label. Small cleanup |
