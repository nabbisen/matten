...
**Document Version:** `1.82.0`
**Date:** 2026-07-17
**Status:** Canonical roadmap updated for the RFC-069 input-mode HTML implementation prepared for review. RFC-068 fixed-demo HTML remains closed after 0.35.0; RFC-069 now has a reviewed policy audit and handoff plus a working-tree implementation for `tools/matten-report --input <csv> --kind data-readiness --select <cols> --format html --output <path>` as a narrow, summary-only, bounded, escaped local artifact. No public report/viz crate, core visualization API, expression tracing, autograd, dependency change in published crates, SVG/Vega-Lite/JSON/notebook/browser scope, version bump, tag, publish, release prep, or companion maturity change is authorized.
...
| **Post-v0.35.0** | Input-mode HTML policy | RFC-069 proposed: local static HTML for `tools/matten-report --input ... --kind data-readiness` prepared for review under summary-only, bounded, escaped, explicit-output rules | Local-tool visualization artifact |
