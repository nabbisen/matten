...
| **Post-v0.38.0** | Pre-v1 feature work | RFC-077 (`train_test_split_seeded` for `matten-mlprep`, closing the RFC-076 §5 maturity caveat without promoting the crate) and RFC-078 (`matten-stats` companion proposal) proceed on the `0.38.x` line while RFC-076 stays accepted-but-unexecuted | RFC-077 under review; no version bump for either theme |
