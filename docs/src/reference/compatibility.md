...
show explicit matched pins such as `matten = "0.47.0"` plus a companion at the
same release.
...
| Non-panicking arithmetic | **Supported** | RFC-129. `try_add` / `try_sub` / `try_mul` / `try_div` return `Result<Tensor, MattenError>`: `Broadcast` on incompatible shapes, `Unsupported` on dynamic tensors — matching `try_dot`/`try_matmul`. The `Add`/`Sub`/`Mul`/`Div` operators delegate to these and keep panicking on the same conditions with byte-identical messages; they cannot return `Result` (`std::ops` trait constraint). Neither the operators nor their `try_` twins bound the result by `MattenLimits::max_elements` for an ordinary case — arithmetic on already-in-memory, already-validated data is not a boundary the limit model bounds (RFC-132). A genuine multiplicative broadcast expansion (result size exceeding the combined size of its two operands) still checks against the default budget, to keep that one case a catchable error rather than an uncatchable allocator abort. |
