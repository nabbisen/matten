...
> **Production-ready candidate (`0.21.x` family).** A small conversion bridge between
...
- **SemVer:** pre-1.0 (`0.x`). A `0.x` minor bump may contain breaking changes;
  patch releases are additive only. Pin the minor (`matten-ndarray = "0.21"`).
- **MSRV:** Rust 1.85 (edition 2024).
- **`matten`:** shares the `0.21.x` family version (RFC-030).
