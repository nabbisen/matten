...
> **Production-ready (`0.39.x` family).** A small conversion bridge between
...
- **SemVer:** pre-1.0 (`0.x`). A `0.x` minor bump may contain breaking changes;
  patch releases are additive only. Pin the release explicitly (`matten-ndarray = "0.39.0"`).
- **MSRV:** Rust 1.85 (edition 2024).
- **`matten`:** released with the `0.39.x` family version (RFC-030). The published
