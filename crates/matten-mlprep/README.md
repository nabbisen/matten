...
> **Production-ready candidate (`0.33.x` family).** Small, transparent, deterministic preprocessing helpers for
...
- **SemVer:** pre-1.0 (`0.x`). A `0.x` minor bump may break and carries migration
  notes; patch releases are additive only. Pin the release explicitly (`matten-mlprep = "0.33.0"`).
- **MSRV:** Rust 1.85 (edition 2024).
- **`matten`:** released with the `0.33.x` family version (RFC-030). The published
