...
> **Beta (`0.20.x` family).** Small, transparent, deterministic preprocessing helpers for
...
- **SemVer:** pre-1.0 (`0.x`). A `0.x` minor bump may break and carries migration
  notes; patch releases are additive only. Pin the minor (`matten-mlprep = "0.20"`).
- **MSRV:** Rust 1.85 (edition 2024). **`matten`:** shares the `0.20` family version (RFC-030).
