...
| ID | Title | Scope |
|---:|---|---|
| 089 | [`0.41.0` — Core Shape and Slicing Release](./proposed/089-0410-core-shape-and-slicing-release.md) | Releases RFC-087 (`repeat`/`repeat_axis`/`tile`/`meshgrid`) and RFC-088 (negative `slice_str` indices) — the first two themes chosen against §1.1's planning baseline. **No blocking precondition**, unlike `0.40.0`: the orphaned-tag defect was repaired during that release and all 100 tags resolve to ancestors of `main`. Triggered by the release-readiness check answering **yes** at RFC-088's disposition, having answered *not yet* at RFC-087's with the condition written down — both answers on consecutive uses, which is the evidence it is a decision and not a formality. 37 version strings across 17 files, measured with `0\.40\b` per the correction recorded at ROADMAP `3.41.0`; the old suffixed pattern finds only 35, missing exactly the two bare-form sites that also need content updates. No `Maturity` section — every label is unchanged, and inventing one to say so invites the silent-promotion reading RFC-067 forbids |
