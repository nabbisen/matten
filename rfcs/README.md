...
| ID | Title | Scope |
|---:|---|---|
| 089 | [`0.41.0` — Core Shape and Slicing Release](./proposed/089-0410-core-shape-and-slicing-release.md) | **Reviewed and accepted 2026-07-31; implemented, pending review.** Releases RFC-087 (`repeat`/`repeat_axis`/`tile`/`meshgrid`) and RFC-088 (negative `slice_str` indices) — the first two themes chosen against §1.1's planning baseline. **No blocking precondition**, unlike `0.40.0`: the orphaned-tag defect was repaired during that release and all 100 tags resolve to ancestors of `main`. Triggered by the release-readiness check answering **yes** at RFC-088's disposition, having answered *not yet* at RFC-087's with the condition written down — both answers on consecutive uses, which is the evidence it is a decision and not a formality. Version bumped and 37 live strings re-measured and retargeted across 17 files (36/16 remaining once Cargo.toml's own site resolved by the bump itself), matching the RFC's figure exactly when measured with the corrected `0\.40\b` pattern. `docs/src/introduction.md` and `public-api-snapshot.md`'s framing sentence both got content rewrites, not just number swaps, per §4. `[0.41.0]` CHANGELOG entry added (Added/Changed/Version; no Maturity section, since no label changed); historical CHANGELOG/ROADMAP/rfcs occurrences untouched. No `Maturity` section — every label is unchanged, and inventing one to say so invites the silent-promotion reading RFC-067 forbids |
