...
| ID | Title | Scope |
|---:|---|---|
| 085 | [Promote `matten-data` to Production-Ready](./proposed/085-promote-matten-data-production-ready.md) | **Reviewed and accepted 2026-07-30; implementation authorized, not yet implemented.** Closes the full-production review RFC-059 §6 deferred. Two of its four stated concerns have moved: streaming was discharged by RFC-082, and the "wide CSV edge-case surface" now has evidence (RFC-082's 4,000-case randomized differential run, zero mismatches); the candidate cycle has run since 2026-06-27, and `matten-data` is no longer the newest companion. Ten of eleven bar signals clear outright; the eleventh — **stable API** — gets an argument rather than a checkmark, because RFC-082 added `CsvBatchReader` days ago and RFC-082 §5 defers nine further streaming items. §5 answers it: the default surface is unchanged across **38 releases**, everything new is feature-gated and off by default, and every deferred item is additive — with the residual async risk recorded rather than argued away. Label, docs and guard only; no code, API, version, or release change, and the RFC-042 scope lock is untouched |
...
| Companion full-production decisions | RFC-057, RFC-058, RFC-059, RFC-067, RFC-080, RFC-084 | `matten-ndarray` and `matten-mlprep` are production-ready (RFC-080 promoted `matten-mlprep`, closing RFC-058 §5.1's Option B exit criterion via RFC-077); `matten-data` and `matten-stats` are both production-ready candidates (RFC-084 promoted `matten-stats`, discharging RFC-081 §3 Exit A); further promotion to full production-ready requires explicit review |
