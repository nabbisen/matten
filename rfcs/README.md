...
| 097 | [Report Demos as Generated Book Pages](./proposed/097-report-demos-on-the-site.md) | Proposed. RFC-093 §8's phase 2, and **two of §8's premises turned out wrong on investigation**. §8 assumed HTML; the tool emits complete `<html>` documents that cannot be book pages, while its Markdown has zero raw HTML and renders as a real page with the site's nav, theme and search — a smaller commitment, which strengthens the RFC-070 answer. And §8 did not know the tool carries the **same flat-list defect** RFC-095 and RFC-096 just removed, in a demo named `shape-flow`, in a **worse** form: it prints only the result, so the rearrangement is absent rather than unclear. Answers §8's product-surface question by distinguishing published **output** from a published **interface**. Generated pages are **committed with a freshness guard**, not git-ignored like the wasm: mdBook creates a missing page empty and exits 0, so a failed generation would deploy five blank pages silently |
