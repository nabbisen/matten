...
| 086 | [`0.40.0` — Feature and Maturity Release](./done/086-0400-feature-and-maturity-release.md) | Prepared and reviewed (approved, no corrections). Family bumped to `0.40.0` in the tree, publishing the accumulated user-facing work of RFC-082–085: `CsvBatchReader`, `covariance_population`/`skewness`/`kurtosis`, and three maturity promotions never before visible to users. **Not yet tagged or published** — separate owner actions, and tagging is blocked by §3, the `0.38.0`/`0.39.0` tags having been orphaned by the history rewrite. §2 records the process failure that let the work accumulate: every RFC correctly said "no release" for its own slice and nothing asked about the accumulation, the mirror of the `0.31.0`→`0.38.0` eight-releases-with-no-content finding. The retarget was **37 strings across 17 files**, one more than §6 measured — the RFC pattern `0\.39\.[0x]` could not see a bare "0.39 release family", and the guard caught it: the third instance in three RFCs of an enforced invariant catching what a one-time sweep missed |
