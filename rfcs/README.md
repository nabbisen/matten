...
| ID | Title | Scope |
|---:|---|---|
| 087 | [`repeat`, `tile`, and `meshgrid`](./proposed/087-repeat-tile-meshgrid.md) | **Reviewed and accepted 2026-07-30; implementation authorized, not yet implemented.** Closes RFC-039 §8's three deferred shape-composition APIs — the first theme selected against §1.1's planning baseline, chosen for what a learner meets early and often (`meshgrid` is where tutorials go on leaving 1-D; `repeat` exists to contrast with broadcasting). Settles the decisions RFC-039 left open: separate `repeat`/`repeat_axis` following the project's own `var`/`var_axis` precedent rather than NumPy's `Option` argument; `tile` accepts `reps` shorter than rank but **rejects** longer, refusing NumPy's silent rank promotion; `meshgrid` takes two rank-1 inputs and returns a tuple with NumPy's `xy` indexing. §6 states the boundary between the two competing principles rather than glossing it — **match the ecosystem where a divergence would be silent, diverge only where it surfaces as an error that teaches**. A real core public-surface change, so `public-api-snapshot.md` moves. No release, no version bump |
