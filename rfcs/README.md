...
| ID | Title | Scope |
|---:|---|---|
| 084 | [Promote `matten-stats` to Production-Ready Candidate](./proposed/084-promote-matten-stats-production-ready-candidate.md) | **Reviewed and accepted 2026-07-29; implemented, pending review.** Discharges RFC-081 §3 **Exit A**, decided by the owner 2026-07-29. **Not label-only, unlike RFC-080:** the audit found one candidate-bar signal genuinely unmet — `matten-stats` was the only published crate with no CI job and no example smoke runs, and its `dynamic`-gated test ran only in the MSRV job. Part 1 closed that gap first (dedicated CI job, four example smoke runs, release-checklist entry — proven independently before the label moved); Part 2 moved the label at every live site found by a sweep-and-classify pass and *inverted* (did not delete) `check-release-docs.sh`'s assertion of `Experimental`, proven able to actually fail by a deliberate-failure test. Answers RFC-080 §7's "not near promotion" objection head-on rather than editing it away: the ddof divergence is substantially resolved by RFC-083, while the absence of usage history is conceded and carried by what the *candidate* rung means. No release, no version bump, no API change; full production-ready is explicitly not claimed |
