...
| ID | Title | Scope |
|---:|---|---|
| 090 | [Histogram — Bin-Selection Policy](./proposed/090-histogram-bin-policy.md) | **Reviewed and accepted 2026-07-31; implementation authorized, not yet implemented.** Resolves the policy RFC-040 §8 left open in v0.21.2 — **the oldest open question in the project** — and adds the function it blocked. **The policy is that there is no automatic bin rule:** `bins` is a required argument, and no Sturges / Freedman–Diaconis / Scott / `"auto"` mode is implemented, because each is a statistical assumption wearing a default's clothing and §1.1's baseline rules out configurable policy. A required argument is the *absence* of a policy, not one — the same shape RFC-082 chose for `batch_rows`. Matches NumPy on the closed last bin, since an open one would silently drop the maximum; diverges on constant input, erroring rather than inventing NumPy's `±0.5` range — both sides decided by RFC-087 §6's silent-vs-visible rule. **Amends RFC-078 §5's `Tensor -> f64` boundary** to "scalar where scalar, a small owned struct where inherently vector-valued, never a `Tensor`" — stated explicitly, and deliberately not unblocking the matrix-wide forms RFC-083 §6 deferred |
