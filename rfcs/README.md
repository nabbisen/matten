...
| ID | Title | Scope |
|---:|---|---|
| 083 | [`matten-stats` Expansion](./proposed/083-matten-stats-expansion.md) | **Reviewed and accepted 2026-07-29; implementation authorized, not yet implemented.** Adds `covariance_population`, `skewness`, `kurtosis` — 3 public functions to 6. Additive only; no new error variant, no new dependency, no feature gate, no version bump, no release, and **no maturity change** (`matten-stats` stays `Experimental`). Settles the estimator convention explicitly: ecosystem default per function, which means `covariance` stays bias-corrected while `skewness`/`kurtosis` are not. Defers histogram (RFC-040 §8 bin policy), matrix/axis-wise forms (would change the crate's `Tensor -> f64` shape), and z-score (belongs to `matten-mlprep`). Sequenced **before** the `matten-stats` promotion RFC so the RFC-057 audit covers the final surface |
