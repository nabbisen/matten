...
| ID | Title | Scope |
|---:|---|---|
| 086 | [`0.40.0` — Feature and Maturity Release](./proposed/086-0400-feature-and-maturity-release.md) | **Reviewed and accepted 2026-07-30; implemented, pending review.** Ships the accumulated user-facing work of RFC-082–085: `CsvBatchReader` (feature-gated), `covariance_population`/`skewness`/`kurtosis`, and the three maturity promotions that have never been visible to users — crates.io still advertises `matten-stats` as `Experimental`. Records the process failure that let it accumulate: every RFC correctly said "no release" for its own slice and nothing ever asked about the accumulation, the mirror image of the `0.31.0`→`0.38.0` eight-releases-with-no-content finding. **Carries a blocking precondition:** the `0.38.0` and `0.39.0` tags were orphaned by the history rewrite and point off-branch, so tagging `0.40.0` first would bury the breakage behind a working newest tag — owner action, blocks tagging not preparation. Version bumped and 36 live strings re-measured and retargeted (matched the RFC's figure exactly); one additional stale reference in `docs/src/introduction.md` found and fixed, missed by the RFC's own 16-file list since it read "current 0.39 release family" rather than a bare version string. `[0.40.0]` CHANGELOG entry added; 132 historical occurrences (CHANGELOG/ROADMAP/rfcs) untouched. §10 proposes closing the cadence gap by folding a release-readiness check into the existing §6.4 disposition checkpoint; not adopted by this release |
