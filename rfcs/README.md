...
| ID | Title | Scope |
|---:|---|---|
| 088 | [Negative Indices in `slice_str`](./proposed/088-negative-slice-indices.md) | **Reviewed and accepted 2026-07-30; implementation authorized, not yet implemented.** Closes RFC-008's `0.1.0` deferral of negative indices — `"-1"` is the last element, `"0:-1"` everything but it. Second theme against §1.1's baseline, and met earlier than anything else on the list: `x[-1]` is among the first things a Python reader tries, and today it is a parse error. **`slice_str` only** — the builder is excluded because `IntoSliceRange` is sealed over five `usize` range types, and adding `isize` impls would make every existing `range(1..3)` call ambiguous, a **source-breaking** change RFC-015's policy does not permit for a convenience feature. Out-of-range negatives **error rather than clamp**, diverging from Python but matching matten's own behaviour for positive out-of-range, and justified by RFC-087 §6's boundary rule since the divergence is visible. Also flags a pre-existing inconsistency it makes easier to hit: `"0:0"` yields shape `[0]`, which the constructor rejects |
