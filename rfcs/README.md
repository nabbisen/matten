...
| 133 | [Matmul Loop Order, and a Benchmark That Can See It](./accepted/133-matmul-loop-order-and-a-benchmark-that-can-see-it.md) | [`133-…-handoff.md`](./handoffs/133-matmul-loop-order-handoff.md) — accepted 2026-09-03, **unblocked** (RFC-127's `math.rs` collision shipped in `0.46.2`). Change A (a 512×512 benchmark case) lands **before** Change B (the `i-j-k` → `i-k-j` interchange), or the ~10× the external audit measured is unmeasurable here. That number is **the audit's, never reproduced on this project's hardware** — verifying it is the point of Change A, and a truthful negative result is a complete outcome. First theme of `0.48.0` |
