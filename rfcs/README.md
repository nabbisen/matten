...
| 111 | [Zero-Sized Dimensions Accepted — Stage 3](./done/111-zero-sized-dimensions-accepted.md) | **Unreleased**. Closes the shape model: `checked_shape_len`'s zero check deleted, the ndarray bridge accepts zero-length axes, and `Display` shows an empty tensor's shape. **Both one-way round-trips now close** — matten could serialise and could `to_arrayd` an empty tensor while reading back neither. **Two mid-work checkpoints each found something this RFC asserted wrongly**: its cascade premise held for only 14 of ~18 sites (`try_linspace` panicked; `repeat`/`repeat_axis`/`tile` had their own guards its §9 already required removing), and its E9 was a probe of a hypothetical tree filed as a present fact — which uncovered RFC-112's regression. Approved after one correction: three surviving false claims, one in production source and the third revision of that same sentence |
| 112 | [`matten-mlprep` Panics on Zero Rows — an RFC-110 Regression](./done/112-mlprep-axis-panic-regression.md) | **Unreleased, and a release blocker for RFC-110.** RFC-110 turned a sentinel leak into an error, which was correct for direct callers and wrong for `matten-mlprep`, which called the *panicking* axis forms precisely because the sentinel made them safe — so a zero-row input panicked through a published API. Found by the implementer during RFC-111's audit, and correctly not fixed there. Proven with two detached worktrees: `Err` before RFC-110, `PANIC` at it, RFC-111 not implicated. **RFC-110's own review missed it because it enumerated behaviours and tests, and no callers** |
| 113 | [The Playground Catches Up](./done/113-playground-catches-up.md) | Ships in no release (`publish = false`). Replaces the playground's hand-rolled copy of core's matmul shape rules with `try_matmul`, which has existed since `0.43.0`, and covers zero-sized inputs RFC-111 newly made typeable. **The task turned on one distinction**: two reimplementations of core behaviour, one now avoidable and one not — no `try_add`/`try_broadcast` exists, so the broadcast duplication stays. Deleting both for consistency was the likeliest defect and did not happen in either attempt. **Implemented twice**: the reviewer destroyed the first implementation with a `git checkout` probe cleanup, producing rule 003 |
| 114 | [`0.46.0` — The Empty-Tensor Release](./done/114-0460-empty-tensor-release.md) | 0.46.0 (released 2026-08-09). Releases RFC-110, RFC-111 and RFC-112, which **must ship together** — RFC-110 alone is a regression and RFC-112 is its fix. **`Changed`-only**, verified: no public item was added, so the API snapshot needed a sentence and **no new row** — the exact inverse of `0.45.0`, which needed four. Its §3 named the falsehood this entry was most likely to contain and the implementation avoided it: **the release must not claim to fix a panic**, since RFC-110 introduced one and RFC-112 removed it inside the same release, so no published version ever had it. Approved with no corrections; the one discrepancy raised was **an error in the handoff** — its occurrence count was measured at one commit and labelled with another, the fourth instance of that single mistake in this sequence |
| 115 | [Playground Input Ergonomics, and a Dynamic Path](./done/115-playground-input-and-dynamic.md) | Ships in no release. Part A fixed the shared parser: an **interior** blank cell was silently dropped — shifting every value after it — and only commas were separators, so a grid pasted as rows failed. The subtlety was that the old filter was not wrong but **too broad**, existing to forgive a trailing comma. Part B added a `try_numeric` demo rather than a slicing one: the single gate in the lifecycle, and the question a learner with a spreadsheet actually has. **The RFC's wasm figure was corrected at review** — its +1.7% measured the bare feature flag, not the change, which is ~+18% |
| 116 | [Render the Diagrams, and Repair Two Unmaintained Records](./done/116-docs-infrastructure-and-stale-records.md) | Ships in no release. RFC-107's two diagrams now render; ROADMAP's release table and the handoffs index are complete. **Two corrections to the RFC came from the implementation**: `mdbook-mermaid install` writes to the book root rather than `docs/theme/`, and E7's count and era were both wrong — 8 missing not 18, RFC-043–054 not RFC-033–042, found by set comparison where the RFC's `grep -c` on a filename pattern silently missed rows. The owner chose **ACCEPT** on a 2.6 MiB bundle that loads on every page, on the reasoning that the cost is fixed rather than per-diagram — so every future diagram is free |
| 117 | [CI Has Been Red Since Before `0.46.0` Was Tagged](./done/117-red-ci-since-0460.md) | No release — no `crates/` change. CI was red for **four consecutive runs spanning the `0.46.0` tag and publish**, and the reviewer authorized both without checking the workflow on the commit just pushed. RFC-111 removed core's zero-sized rejection, so `matten-report` stopped erroring on a header-only CSV — a **shell test in a workspace-excluded tool**, which no `scripts/*.sh` ran and `cargo test --workspace` cannot reach. Adds the ninth guard, deriving its command list from `test.yaml` and erroring loudly if that derivation ever yields zero. Also adds the `matten-data` bullet `[0.46.0]` omitted while changing that crate's published behaviour. **The second case could not be inverted** — it protects the property that a pre-write failure leaves an existing output untouched, so it was repointed at a nonexistent input rather than deleted |
| 118 | [Confirm CI Green Before Tagging](./done/118-confirm-ci-before-tagging.md) | Documentation only; no release. Adds the step missing when `0.46.0` was tagged and published across **four consecutive red CI runs**: the checklist had **zero** CI-checking references, so the omission was structural rather than a skipped step. Approved after one correction the implementer had themselves flagged — the new section was first placed under *Additional gates for minor releases*, which would have entitled a patch release to skip a blocking step, **reintroducing the exact failure by location rather than by text**. Moved to its own top-level section, taking the pre-existing `Publishing` subsection with it (content byte-identical), and carrying a sentence recording *why* it lives outside that heading so a future reorganisation cannot undo it |
| 119 | [Corrections to the Published Surface](./done/119-published-surface-corrections.md) | **Unreleased** — the first `crates/` change since `0.46.0`. Fixes a panicking example shipped in **83 releases since `0.17.0`** (`examples/` is packaged, so it was inside the crate users download), plus three false published statements: the crate-root doc had called dynamic slicing *guarded* since RFC-102 shipped it in `0.44.0` **using the very CoW view it deferred to**, and two READMEs still denied the zero-sized dimensions RFC-111 accepted. Closes the gap that hid the panic — `cargo check --examples` proves compilation, nothing proved execution, leaving **20 of 78 examples never run**; now 76 of 78, the two exclusions documented in their own headers. `tools/matten-playground` gains its first `cargo test`/`clippy` CI step, 43 tests that no workflow had ever run. **Review required one correction of the same class the RFC exists to remove**: the first fix to `stats.rs` asserted z-score was *"unimplemented anywhere in the family"* when it ships as `matten-mlprep::standardize_columns` — a claim about one crate's capability, wrong, inside another crate's docs. Two of the reviewer's own figures were corrected by the implementer (six `docs/src/` pages in `2ac99bb`, not eight; the sentence named six functions, not four) |
| 120 | [Patch Releases Cover Published Documentation](./done/120-patches-cover-published-documentation.md) | No release — amends one RFC document. RFC-094 §4.1 defined a patch as *"correctness fixes to already-published crate **code**"* and §4.3 excluded only documentation *"that does not reach crates.io"*, so a false statement in published rustdoc or a packaged README fell through **both**, and §4.2 — which defined triggers and, unlike §4.1, carried no contents clause at all — caught it on the 28-day floor. The literal policy therefore routed a one-line correction into a **minor release a month later**. Both of the policy's own mechanisms already pointed the other way: §4.3's `cargo package --list` test (the one that declined `0.42.1`) puts those files inside published packages, and §4.1's rationale — *"a user hitting a wrong answer should not wait"* — describes a reader of a false capability claim exactly. Amended by blockquote per RFC-095's precedent, originals quoted verbatim, **31 insertions and 0 removals**; §4.3, the triggers and the 28-day figure untouched |
| 121 | [`0.46.1` — The Published-Surface Patch](./done/121-0461-published-surface-patch.md) | **The project's first patch release since `0.28.5`** and the first ever under RFC-094, whose §3 had recorded that patches *"quietly stopped existing"*. Publishes RFC-119's corrections, which reach no reader until a publish happens — docs.rs renders the *published* rustdoc and crates.io the *published* README. 21 live pins across 14 files; **the 13 `0.46.x` family references correctly did NOT move**, the trap all three preceding releases had set by being minors. Three `0.46.0` occurrences deliberately preserved, chief among them `release-checklist.md:276` — RFC-118's record that `0.46.0` was tagged across four red CI runs, which a blind retarget would have rewritten into a claim about `0.46.1`. Inverts RFC-109 twice: no new `public-api-snapshot` row and no `introduction.md` rewrite. **Review required one correction, and the error was the reviewer's**: this RFC's §6 dated `matten-stats`' first release to `0.38.0` when it shipped at `0.39.0` — a false statement in the release note whose whole subject is false statements. The implementer had followed the design authority faithfully, which is the correct behaviour; review is where such an error belongs |
| 122 | [Project Logo — Assets and the Unpublished Surfaces](./done/122-project-logo.md) | No release — no `crates/` path touched; the docs.rs `html_logo_url` is **ineligible for a patch** under RFC-094 §4.1 as amended by RFC-120 (a logo is not a correctness fix) and waits for `0.47.0`. Split execution: assets by the high-capability model, placement by the implementer. **The candidate art failed below 64px** — mush at 32, unidentifiable at 16 — so the small mark was hand-authored as SVG with widened gaps and raised saturation until 3×3 held at **16px**; sampling showed two of the three face colours were near-white and would have merged. Removing the white surround also lifted the cream plate, making a dark variant unnecessary — one transparent file reads better on dark than light — so the RFC's `<picture>` instruction was **amended after implementation**. Assets total 153 KB against the candidates' 2.3 MB. Review required one correction: the alt text described the artwork, which is both the wrong kind of alt text for a logo and **a statement that goes false when the logo changes** — the defect class the audit had just found four of. Now `alt="matten"` |
| 124 | [A Simpler, Softer Mark](./done/124-simpler-softer-logo.md) | No release — five asset files, **no markup change at all**, because every filename stayed identical and RFC-122's placement kept working untouched. Replaces the 3×3×3 cube-in-a-palm with a **2×2×2 cube in a cradle** on the owner's judgement that the mark read *too complicated and intimidating* against a family positioning. The evidence had been sitting in RFC-122's own E5, which measured the mark failing below 64px and named 27 facelets as the cause — read at the time as a rendering problem needing a second simplified mark, when it was equally a design signal: the density that defeats a 32px grid is the density that reads as complicated at any size. `[2,2,2]` is not a watered-down cube but the **smallest non-trivial rank-3 tensor**; a plain undivided cube was tried and rejected for ceasing to mean *array*. One SVG is now the source of truth for every size, retiring RFC-122's two-mark split. **RFC-122's `alt` correction paid for itself three days later** — had the hero still described the artwork, this change would have made the README false |
| 125 | [The Owner's Simplified Mark](./done/125-the-owners-simplified-mark.md) | No release — five asset files, no markup change. Adopts the owner's regenerated artwork: the **original composition** — cream plate, two tan sweeps, warm sun, sage palm — with the cube simplified from 27 facelets to **8 cells**. Supersedes RFC-124 three days after it closed, and §2 records why: RFC-122's measurement justified simplifying *the cube*, and a reviewer generalised it into removing the surroundings too — an error of scope, not taste. Of the two candidates supplied, the **subdivided** cube was chosen over a plain one: a plain cube reads as a **crate** — the Rust package unit and crates.io's own iconography — pointing at the wrong concept entirely; and measured at 16/32/64px it holds *better*, its warmer faces keeping separation where muted pastels merge. **Two deviations at implementation** (§12.1): the prescribed wider-fuzz fix for the plate edge was proven impossible — plate and surround differ by ~2%, so no threshold separates them and higher values ate the sweeps — so the boundary was located geometrically instead; and consequently the plate is **retained** as a rounded tile rather than removed as in RFC-122 and RFC-124 |
| 126 | [The Favicon Should Be the Mark](./done/126-favicon-should-be-the-mark.md) | No release — three files, no markup. The `.svg` favicon is served under `rel="icon"`, which modern browsers use, so **the browser tab was showing a drawing that was not the project's mark** — RFC-125's hand-authored lockup rather than the artwork. `assets/matten-mark.svg` was referenced by **nothing** outside RFC records and is deleted; a logo-shaped file that is not the logo is a trap for the next reader. Reverses RFC-125 §6, which had claimed that at 16–32px *"the plate, sun and palm do not register as distinct forms"* — **asserted, not rendered, and false**: side by side at 32px the full artwork is *more* legible, the plate supplying a silhouette and the sun an accent. Third instance in the logo sequence of a claim about the artwork being reasoned instead of rendered, and §3 records the rule that generalises: **render it and look before writing the justification.** States plainly that it does **not** deliver a scalable vector logo — the artwork is raster, so a vector could only approximate it, and the approximation is what was removed |
| 127 | [The Unbounded Dimension](./done/127-the-unbounded-dimension.md) | **Released in `0.46.2`** (2026-09-03). Closes the external architect audit's Critical: a **36-byte JSON document aborted the process, uncatchably**, and `try_matmul` returned a `Tensor` whose `shape.product()` was `3.4e38` against a `data.len()` of 1. Root cause traced to RFC-111 — rejecting zero dimensions had *incidentally* bounded every dimension, and removing that rejection left `checked_shape_len` guarding only the product, which a zero makes unreachable. Also fixes a **silent wrong answer**: `slice().index(usize::MAX)` returned the last row where `index(9)` correctly errored. **The RFC's own instructions were wrong in three places, found by the implementer deriving rather than trusting**: the cast list omitted `SliceBuilder::index` — the only site the flagship reproduction reaches, so following it literally would have passed every acceptance criterion with the defect alive — the construction-site count was 47 not "~31", and E4's release-only evidence missed a debug-mode panic in a `Result`-zone function. Review then found the fix rested on an **unstated invariant** (that `max_elements <= isize::MAX`), the same class of omission that caused the original defect; corrected by capping the effective bound at `isize::MAX / 8` |
| 134 | [`0.46.2` — The Unbounded-Dimension Patch](./done/134-0462-the-unbounded-dimension-patch.md) | Ships RFC-127's Critical — an **uncatchable process abort reachable from a 36-byte JSON document**, present in every published version since `0.17.0`. 21 pins across 14 files; the **8** `0.46.x` family references correctly did not move, and the count had shifted from 13 when the crate table's Version column became badges — the trap the handoff named, and the implementer derived 8 independently. Carries a **`### Security`** heading, the project's fifth. **Review required one correction**: the security bullet claimed CSV as an attack vector, which it is not — every CSV path derives its shape from counts of actually-parsed data, never from a declared shape, so only JSON can declare an enormous shape in a tiny document. The implementer went further than asked and stated *"CSV is not affected"* explicitly, which is what a reader auditing their exposure actually needs |
| 129 | [Result-Zone Arithmetic](./done/129-result-zone-arithmetic.md) | **Released in `0.47.0`** (2026-09-03). Adds `try_add`/`try_sub`/`try_mul`/`try_div`, completing the two-zone error contract for the four operators that lacked a recoverable twin. Landed with RFC-132 because that RFC's Option A decision removed the budget check its original test asserted. **A near-miss worth recording**: routing the dynamic guard through an existing shared helper silently changed a published panic message, on the one arithmetic path no test pinned — caught by hand, reverted, and now pinned by an exact-text test |
| 128 | [Property Testing](./done/128-property-testing.md) | **Released in `0.47.0`** (2026-09-03). Adds `proptest` as a **dev-dependency only** (`cargo tree -e normal` unchanged) and writes four invariants as properties — the shape/data invariant that `0.46.2` existed because nobody had written down, the index round-trip, broadcasting, and slice bounds. **Both properties were proven able to fail** by reverting RFC-127's actual guards, the abort one reproduced in an isolated subprocess under `ulimit` because observing it casually would take down the test runner. **The most valuable result is a finding against the technique itself**: P4's property, run against the reverted bug, *passed* — `any::<usize>()` never sampled a top-bit-set value in 256 cases — so a deterministic regression test was added beside it. Properties and example tests are complements, not substitutes. The implementer also self-reported two bugs in their own test assertions, both caught by properties failing. Its §7 packaging question resolved definitively: test files **do** ship — 32 of 136 files, 44% of the crate's source — which is now recorded as a separate coherence candidate |
| 131 | [Documentation Corrections, Batched](./done/131-documentation-corrections-batch.md) | Book half live on push; the five rustdoc corrections **shipped in `0.47.0`** (2026-09-03). Splits the external audit's documentation findings by **what a release can carry** — `docs/src/**` reaches zero packages, `crates/**/*.rs` is packaged. The quick-start no longer hands newcomers `default-features = false`, the profile on which `Tensor::from_csv` does not exist; a troubleshooting page exists where there was none. **Its strongest work was refusing the handoff's own error strings**: the handoff wrote *"no method named `from_csv`"*, so they built against a lean dependency and used rustc's actual E0599 wording — *associated function*, because that is what it is — and chose the operator-panic form of the broadcast message over the plain `Display`, since the former is what `&a + &b` prints. D-6 and D-8 were **verified, not edited**: RFC-127's E2/E4/E5 probes re-run to confirm the crate-root safety promise is now true, and the sentence left character-for-character unchanged |
| 132 | [The Limit Policy, Made Coherent](./done/132-the-limit-policy-made-coherent.md) | **Released in `0.47.0`** (2026-09-03). Decided **Option A, boundary-only**, then applied it: *limits bound any allocation that can exceed the combined size of the inputs determining it.* **The site list was wrong twice and the rule right twice.** First, the list would have removed guards from `matmul`/`outer`/broadcast — reintroducing an uncatchable abort reachable from two ordinary 1M vectors and a `+`, six days after `0.46.2` shipped to close that class; caught by the implementer *before writing any code*. Second, "keep the broadcast guard unchanged" contradicted the RFC's own worked example, since an unconditional guard rejects a same-shape 2M addition; resolved by guarding only genuine multiplicative expansion. Also makes `max_parse_bytes` enforced rather than the documented no-op it had been |
| 135 | [`0.47.0` — The Result-Zone and Limit-Model Release](./done/135-0470-result-zone-and-limit-model-release.md) | **Released, tagged and published 2026-09-03.** All five crates live at `0.47.0`, verified against the **sparse index** rather than the JSON API, none yanked. Carries RFC-128/129/131/132. **The tag's placement needed an argument and got one**: three commits touching zero `crates/` paths had landed on top of the *Prepare* commit before authorization arrived, so `git diff --name-only d1f02eb..HEAD -- crates/` was run to prove the published content byte-identical either way before tagging the ancestor. **The CI gate was confirmed against the pushed SHA specifically** — `gh run list --commit` silently returned nothing, so both workflows' `head_sha` were read from the API; a green run belonging to an earlier commit is the exact failure RFC-118's gate exists to prevent |
| 123 | [Move the Per-Crate Badges into the Crate Table](./done/123-crate-table-badges.md) | **Change D only** — A/B/C were performed by the owner. The guard it replaced watched version strings Change C had already deleted: it matched **zero rows and could never fail again**, the vacuous-guard failure RFC-117 exists to forbid. The replacement derives the crate list from `crates/` **on disk**, so the sixth crate is covered the day it is created rather than the day someone remembers. Proven able to fail by deleting a badge, and independently reproduced at review rather than read from the transcript |
| 133 | [Matmul Loop Order, and a Benchmark That Can See It](./done/133-matmul-loop-order-and-a-benchmark-that-can-see-it.md) | **Released in `0.48.0`** (2026-09-07), its first theme. **~10× on 512×512** (159.34 ms → 15.67 ms; independently reproduced at review as 9.9× against a from-scratch `i-j-k` reference), matching the external audit's number on different hardware. **The RFC's own float claim was wrong and the implementer disproved it rather than complying**: §5.1 said results *"change in the last bits"*, but the interchange is **bit-identical** — both orders add the same terms in the same `k` sequence into each cell, and IEEE 754 `+` does not care whether the accumulator sits in a register or in `out[i*p+j]`. Verified at review across six adversarial cases (catastrophic cancellation, a 1024-term mixed-magnitude run, vector remainders, denormals, `0.0 * inf`) in **both** profiles. The reasoning error was general-rule-without-instance-check: interchange changes summation order for *tiling and parallel reduction*, transforms that **split** a reduction — not for one that keeps `k` as the per-cell sequence. So the changelog makes the **stronger** claim (*no numeric change*), and it is still a minor on the surviving reason: RFC-094 §4.1 confines patches to correctness fixes, and being slow was not a defect. **Zero tests needed changing** |
| 136 | [The Axis-Reduction Hoist](./done/136-axis-reduction-hoist.md) | **Released in `0.48.0`** (2026-09-07), its second theme, which completed RFC-094 §4.2's two-theme trigger. Hoists the per-element coordinate arithmetic out of `axis_reduce` and `nan_axis_reduce`; the cost was **four heap allocations per input element**, over four million to perform one million additions. **Bit-identical** across ranks 1-4 and every axis, verified at review against an independent reconstruction — and unlike RFC-133 that was derivable *before* implementation, since nothing is reordered and the guarantee therefore covers non-associative `min`/`max` too. `nan_axis_reduce` gained the `check_shape` guard it lacked: **unreachable today**, because `min_axis` rejects a zero-length reduced axis upstream, but unreachable for a reason living in another function. **The review's finding was about measurement**: 81×, 110× and 199× are all true of this change on the same machine, because the old path's four million allocations make the *baseline* swing ~2× with allocator warmth and ~2.6× with machine load while the hoisted path stays stable. The changelog claims the **floor** (~10×), never a headline multiplier |
| 137 | [`0.48.0` — The Performance and Metadata Release](./done/137-0480-performance-and-metadata-release.md) | **Released, tagged and published 2026-09-07.** All five crates live at `0.48.0`, verified against the sparse index. **The first release changing nothing a user's code can observe except speed** — zero public items added or removed, both optimisations bit-identical. Its lasting finding is about measurement: RFC-136 measured 81×/110×/199× for the *same change on the same machine*, because the old path's four million allocations make the **baseline** swing while the hoisted path is stable, so the changelog claims a **floor** and never a multiplier. A guard defect was found and deliberately left for its own RFC: the stale-version patterns are line-based and cannot match `public-api-snapshot.md`'s wrapped sentence — the file their own comment names — which is how `0.39.0` shipped claiming to document the `v0.38` API |
| 130 | [Repository Governance Files](./done/130-repository-governance-files.md) | **Closed 2026-09-07.** A/B landed 2026-09-03; **Change C shipped in `0.48.0`**, three releases after §6 aimed it at `0.47.0`'s slice. Held in `accepted/` for four days with two thirds built rather than moved to `done/` with Change C outstanding — the folder is the only status this project keeps. **§6's wording is the lesson**: naming a specific release made the instruction go stale in three days, where *"the next minor"* would have survived |
