...
| ID | Title | Scope |
|---:|---|---|
| 084 | [Promote `matten-stats` to Production-Ready Candidate](./proposed/084-promote-matten-stats-production-ready-candidate.md) | **Reviewed and accepted 2026-07-29; implementation authorized, in progress.** Discharges RFC-081 §3 **Exit A**, decided by the owner 2026-07-29. **Not label-only, unlike RFC-080:** the audit finds one candidate-bar signal genuinely unmet — `matten-stats` is the only published crate with no CI job and no example smoke runs, and its `dynamic`-gated test currently runs only in the MSRV job. Part 1 closes that gap; Part 2 moves the label and *inverts* (does not delete) `check-release-docs.sh`'s assertion of `Experimental`. Answers RFC-080 §7's "not near promotion" objection head-on rather than editing it away: the ddof divergence is substantially resolved by RFC-083, while the absence of usage history is conceded and carried by what the *candidate* rung means. No release, no version bump, no API change; full production-ready is explicitly not claimed |
| 076 | [v1.0 Release Preparation](./proposed/076-v1-release-preparation.md) | Reviewed and accepted (GO, no conditions); its inventory refreshed by RFC-081 (five-crate family, matten-mlprep production-ready, matten-stats row added, RFC-081 §3 precondition added). RFC-081's precondition is now on a path to resolution — `matten-stats` takes **Exit A (promotion)** — but the promotion has not landed, so this RFC remains blocked and no implementation is authorized. v1.0 is not currently wanted |
