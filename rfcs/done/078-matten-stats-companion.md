...
**Status:** Implemented — reviewed (GO, no conditions, `matten-rfc078-implementation-review-v0.1.md`), committed `7f1cbba`; fifth published crate, Experimental maturity. **Published at `0.39.0`** (RFC-079) despite RFC-079 §3's decision to defer this crate's first publication pending an external read of §4.1's `ddof = 1` policy — see RFC-079 §13 and the `0.39.0` post-release alignment handoff. That external read remains open; it now informs whether a future change is warranted rather than gating a publication that has already happened
