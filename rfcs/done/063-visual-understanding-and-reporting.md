...
**Status:** Implemented for `0.29.0` with scope clarifications; Phase 1 visual docs shipped in `0.29.0-pre.1`; Phase 2 canonical example reports shipped in `0.29.0-pre.2`; optional `matten-mlprep` visual-standardization example shipped in `0.29.0-pre.3`; Phase 3 local-tool slices shipped in `0.29.0-pre.4` for `matten-data` readiness, `0.29.0-pre.5` for demo-only shape-flow, `0.29.0-pre.6` for demo-only dynamic-readiness, and `0.29.0-pre.7` for demo-only mlprep-standardization; final docs link/navigation cleanup included in `0.29.0`; public report/viz crates and richer output formats remain deferred to later RFCs or explicit amendments
...
Phase 1 through the accepted local-tool Phase 3 slices are complete in the
`0.29.0` family. The remaining Phase 4 idea (published report/viz companions)
is explicitly closed for `0.29.0` and deferred to a later RFC or explicit
amendment after local tooling proves durable value.
