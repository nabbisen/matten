...
HELP_SHA256="78b2e3342c78847fe3976d2e71da1735d6e70f57ddb9d65f0be52b202f23f342"
...
assert_fingerprint help "$CASE_STDOUT" 1509 "$HELP_SHA256"
...
assert_nonzero_status missing_parent
assert_empty missing_parent stdout "$CASE_STDOUT"
[[ "$(wc -l <"$CASE_STDERR" | tr -d '[:space:]')" == 1 ]] ||
    fail "missing_parent: expected exactly one stderr line"
grep -q '^matten-report error: ' "$CASE_STDERR" ||
    fail "missing_parent: stderr did not begin with the process error prefix"
[[ "$(tail -c 1 "$CASE_STDERR" | od -An -t x1 | tr -d '[:space:]')" == 0a ]] ||
    fail "missing_parent: stderr was not newline-terminated"
